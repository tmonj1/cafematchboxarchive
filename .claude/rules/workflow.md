...
コードの追加・修正をする場合、ユーザーが明示的に別の指示をしない限り、**issue作成からPRのマージまたはクローズまで、まず以下のフロー全体を自動的に最後まで実行する**。
...
- 以下のいずれかの条件に合致したらPRの監視を解除して終わる
  - PRがマージされた
  - レビューアが追加のコメントはない (no new comments) とのコメントを書いた
  - 10分以上コメントの追加がない
