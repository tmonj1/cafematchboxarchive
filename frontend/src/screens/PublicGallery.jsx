  ...
  return (
    <div style={{ background: theme.bg, minHeight: isDesktop ? '100vh' : '100%' }}>
      <TopBar title="CAFE MATCHBOX ARCHIVE" theme={theme}
        left={<div style={{ fontFamily: '"Work Sans", sans-serif', fontSize: 10,
          color: theme.sub, letterSpacing: '0.15em' }}>蒐集 / 2026</div>}
        right={user ? (
          ...
          <button onClick={() => setShowLogin(true)} style={{
            ...
            color: theme.ink, cursor: 'pointer', fontWeight: 500 }}>ログイン</button>
        )}
      />
      ...
      <div style={{ padding: '28px 16px 8px' }}>
        <div style={{ fontFamily: '"Work Sans", sans-serif', fontSize: 10,
          ...
          A COLLECTION OF JAPANESE CAFE MATCHBOXES
        </div>
        <div style={{ fontFamily: '"Noto Serif JP", serif', fontSize: 22, lineHeight: 1.5,
          color: theme.ink, fontWeight: 500 }}>カフェマッチボックス<br />コレクション</div>
        <div style={{ fontFamily: '"Noto Sans JP", sans-serif', fontSize: 11,
          color: theme.sub, marginTop: 12, lineHeight: 1.7 }}>消えゆく街の記憶を、小さな箱のなかに。</div>
      </div>
      ...
      <div style={{ padding: '40px 16px 80px', textAlign: 'center',
        ...
        <a onClick={() => nav('about')} style={{ fontFamily: '"Noto Sans JP", sans-serif',
          fontSize: 12, color: theme.sub, textDecoration: 'underline',
          cursor: 'pointer', textUnderlineOffset: 4 }}>このサイトについて</a>
      </div>
